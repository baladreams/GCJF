...
class Case:
  ...
  def _parse_contestant_query(self, bitstring):
    ...
    if len(bitstring) != self.__N:
      return (None, _ERROR_MSG_INVALID_TOKEN+" "+bitstring)
  ...
  def _parse_contestant_answer(self, tokens):
    ...
    if len(tokens) != len(self.__bad_set):
      return (None, _ERROR_MSG_INCORRECT_ARG_NUM+" Response:"+self.responses)
    try:
      contestant_answer = list(map(int, tokens))
    except Exception:
      return (None, _ERROR_MSG_INVALID_TOKEN+" "+contestant_answer)
  ...
  def _parse_contestant_input(self, response):
    ...
    if not re.match("^[\s0-9-]+$", response):
      return None, _ERROR_MSG_INVALID_TOKEN+" Response:"+self.responses
    ...
    for queries in range(self.__max_num_tries + 1):
      ...
      if type(contestant_input) is str:
        ...
        if queries == self.__max_num_tries:
          ...
          answer=self._answer_query(contestant_input)
          self.responses=self.responses+","+answer
          print(answer)
          sys.stdout.flush()
      else:
        # Answer
        ...
        if set(contestant_input) == self.__bad_set:
          ...
          return _WRONG_ANSWER_MSG+" Response:"+self.responses
    return _QUERY_LIMIT_EXCEEDED_MSG

def getTestCases(test_number):
  F = (10, 5)[test_number]
  # You can edit or add your own test cases here.
  cases = [Case([1, 2, 3], 4, F), Case([2, 3, 5], 6, F), Case([1000], 1024, F),Case([866,955,956,957,958,959,960,961,962,963,964,965,966,967,968,969,970,1000,1001,1002,1003,1004], 1024, F)]
  return cases
